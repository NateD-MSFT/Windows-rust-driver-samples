...
use crate::{osrusbfx2::*, trace::EVENT_LOGGER};
...
extern "system" fn driver_entry(
    ...
    let device_add: PFN_WDF_DRIVER_DEVICE_ADD = Some(device::osr_fx_evt_device_add);
    ...
    unsafe {
        ...
        (*wdf_attributes).EvtCleanupCallback = Some(device::osr_fx_evt_driver_context_cleanup);
    }
    let status = unsafe {
